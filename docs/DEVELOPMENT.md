# Remove All Appearances — development notes
...
Every appearance in the active display state. In a part that is face,
feature, body and part level, including colours applied by hand. In an
assembly it is component appearances only.

It asks for confirmation first, because it destroys work that cannot be
recreated and leaves no undo record.
